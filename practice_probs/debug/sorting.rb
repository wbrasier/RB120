...
class Length
  ...
  def <=>(other)
    case unit
    when :km  then return value <=> other.as_kilometers.value
    when :mi  then return value <=> other.as_miles.value
    when :nmi then return value <=> other.as_nautical_miles.value
    end
  end
